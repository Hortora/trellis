  ...
  return new Promise((resolve, reject) => {
    ...
    const attempt = () => {
      if (Date.now() >= deadline) {
        reject(new Error(`Sidecar did not start within ${timeoutMs}ms`));
        return;
      }
...
function getJarPath(isPackaged, resourcesPath) {
  if (isPackaged) {
    return path.join(resourcesPath, 'quarkus-app', 'quarkus-run.jar');
  }
  return path.join(__dirname, '..', 'sidecar', 'target', 'quarkus-app', 'quarkus-run.jar');
}
